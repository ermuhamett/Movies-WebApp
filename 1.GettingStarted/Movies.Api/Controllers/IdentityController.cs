...
public class IdentityController:ControllerBase
{
    ...
    public IActionResult GenerateToken([FromBody] TokenGenerationRequest request)
    {
        ...
        var claims = new List<Claim>
        {
            ...
            new(JwtRegisteredClaimNames.Sub, request.Email), // Тема (subject) токена, обычно это email пользователя
        ...
        foreach (var claimPair in request.CustomClaims)
        {
            ...
            var valueType = jsonElement.ValueKind switch // Определяем тип значения утверждения на основе его JsonValueKind
            {
        ...
        {
            Subject = new ClaimsIdentity(claims),// Устанавливаем идентификатор субъекта (Subject) с утверждениями (claims)
